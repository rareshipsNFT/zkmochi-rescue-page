...
export function RescueMochi() {
	...
	const {
		...
		isSuccess: isApprovedForAllSuccess,
	...
	return (
		<div className="bg-[#f1b0ff] text-black space-y-[20px] lg:w-[400px] p-[20px] rounded-lg">
			<Grid.Container
				gap={2}
				justify="center"
			>
				...
				>
					<input
						...
						className="rounded-lg w-full bg-[#ffffff82] border-2 border-[#ffffffb5] focus:outline-none py-[5px] px-[10px]"
					/>
				</Grid>
				<Grid
					xs={24}
				...
				<Grid
					...
					justify="center"
				>
					{!isApproved && !isLoadingSetApprovalForAll && (
						<Button
							type="secondary"
							onClick={approve}
				...
				<Grid
					...
					justify="center"
				>
					{isSuccessSetApprovalForAll && (
						<div>Approval Transaction Hash: {dataSetApprovalForAll?.hash} </div>
					)}
					{isSuccessRescue && (
						<div>Rescue Transaction Hash: {dataRescue?.hash}</div>
					)}
				</Grid>
				<Grid
					xs={24}
					justify="center"
				>
					{isErrorSetApprovalForAll && (
						<div>Error: {errorSetApprovalForAll?.message}</div>
					)}
					{isErrorRescue && (
						<div>Error: {errorRescue?.message.split('Contract Call')[0]}</div>
					)}
				</Grid>
			</Grid.Container>
		</div>
